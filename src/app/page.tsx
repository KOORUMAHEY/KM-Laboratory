...
import type { LucideIcon } from 'lucide-react'; // Import the type for our map

// CHANGE 2: Import only the icons you need. This is great for performance.
import {
  ArrowRight,
  BookOpenCheck,
  FlaskConical,
...
// Create a type-safe and secure mapping from string names (from your DB) to actual icon components.
...
export default async function Home() {
  ...
  return (
    // CHANGE 1: Add a consistent container for page layout and spacing.
    // This matches the structure of your About page and ensures content isn't stuck to the edges.
        ...
        {categories.length > 0 ? (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {categories.map((category, index) => {
              // Now we safely get the icon from our map, with a reliable fallback.
              // No more `@ts-ignore` needed!
              const Icon = iconMap[category.icon || ''] || BookOpenCheck;
              ...
              return (
                <Link
                  ...
                  // CHANGE 3: Improved accessibility for screen readers
                  aria-label={`View experiments in the ${category.title} category`}
                >
                  <Card className="h-full transition-all duration-300 ease-in-out hover:shadow-lg hover:-translate-y-1 flex flex-col bg-card/50 backdrop-blur-sm hover:bg-card/80">
