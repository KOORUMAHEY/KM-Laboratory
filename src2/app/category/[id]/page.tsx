...
export default async function CategoryPage({ params }: CategoryPageProps) {
  const category = await getLabCategoryById(params.id);
  const experiments = await getLabExperiments(params.id);
    ...
    <div className="space-y-8 animate-fade-in-up">
       <div className="flex items-start justify-between">
        <div>
          ...
          <LetterGlitch text={category.title} className="text-3xl font-bold" />
